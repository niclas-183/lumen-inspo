...
jeder Karte. Variante 3 nutzt dieselbe Serie als Produktmaterial in
Phone-Frames (Hero: `01` + `03`; Galerie `#v3-programm`: `02`, `05`, `04`) —
auf ausdrückliche Entscheidung; der Ehrlichkeits-Credit steht bei jedem
Vorkommen darunter („App-Store-Serie als Dichte-Referenz — Bevel, nicht
Lumen."). Marke und Screenshots gehören Starlight Tech LLC; keine
...
Für Variante 3 zusätzlich Nunito Sans als Variable Font mit den Achsen
`opsz 6..12`, `wdth 75..125`, `wght 200..1000`, `YTLC 440..540` (derselbe
`<link>`); die Seite fährt `font-variation-settings: "YTLC" 500, "wdth" 100`.
Lora 600 trägt dort alle Überschriften. SIL Open Font License.
...
`currentColor`, viewBox `0 0 24 24`, Klasse `.sys-ic` / `.sys-ic-sm`:
...
|---|---|
| `meditating.svg` | V3 Hero, unter der Trust-Note |
| `sitting-reading.svg` | V3 Beweisstück (`#v3-beweis`) |
| `loving.svg` | V3 Datenschutz (`#v3-datenschutz`) |
